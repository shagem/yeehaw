(() => {
  ...
  fetch("https://api.themoviedb.org/3/trending/movie/day?language=en-US", options).then((response) => {
    if (!response.ok) {
    ...
    trendingMovies.forEach((movie) => {
      ...
      if (roundedRating >= 7) {
        ratingColorClass = "text-primaryGreen";
      } else if (roundedRating >= 4.5) {
        ratingColorClass = "text-primaryYellow";
      } else {
        ratingColorClass = "text-primaryRed";
      }
